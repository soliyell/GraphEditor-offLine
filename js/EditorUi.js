 ...
 */EditorUi.prototype.open = function() {
  ...
  this.editor.setStatus(mxUtils.htmlEntities('按Ctrl+S保存'));
  var thisxml = '';
  var thisname = '';
  try {
    if(window.opener != null && window.opener.openFile != null) {
      ...
      // 新建
      thisname = 'Drawing0.xml';
      thisxml = localStorage.getItem(thisname);
    }
    if(thisxml) {
      try {
        var doc = mxUtils.parseXml(thisxml);
        this.editor.setGraphXml(doc.documentElement);
        this.editor.setModified(false);
        this.editor.undoManager.clear();
        if(thisname != null) {
          this.editor.setFilename(thisname);
 ...
 * 展示缓存中的文件
 */
EditorUi.prototype.openLocal = function() {
  // Closes dialog after open
  window.openFile = new OpenFile(mxUtils.bind(this, function(cancel) {
    this.hideDialog(cancel);
  }));
...
EditorUi.prototype.saveFile = function(forceDialog) {
  ...
  var text = localStorage.getItem('Drawing0.xml');
...
EditorUi.prototype.save = function(name) {
  if(name != null) {
    ...
    try {
      ...
      var title = nowDate.getHours() + ':' + nowDate.getMinutes() + ':' + nowDate.getSeconds();
      localStorage.setItem(name, xml);
      this.editor.setStatus(mxUtils.htmlEntities() + title + ' 保存成功！');
...
EditorUi.prototype.createKeyHandler = function(editor) {
  ...
  keyHandler.isEventIgnored = function(evt) {
    ...
    return (!this.isControlDown(evt) || mxEvent.isShiftDown(evt) || (evt.keyCode != 90 && evt.keyCode != 89 &&
      evt.keyCode != 188 && evt.keyCode != 190 && evt.keyCode != 85)) && ((evt.keyCode != 66 && evt.keyCode != 73) || !this.isControlDown(evt) || (this.graph.cellEditor.isContentEditing() && !mxClient.IS_FF && !mxClient.IS_SF)) &&
